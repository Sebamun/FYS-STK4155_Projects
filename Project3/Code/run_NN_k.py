...
from plot import plot_bias_accuracy, plot_R2
...
def run_kfold_DNN(act_funcs):
    ...
    for i in range(len(act_funcs)):
        ...
        pred = np.ravel(pred)
        target_data = np.ravel(target_data)
        R2[i,:] = r2_score(target_data, pred)

    plot_bias_accuracy(loss, val_loss, accuracy, val_accuracy, N_epochs, act_funcs)
    plot_R2(R2, N_epochs, act_funcs)
