...
from common import FrankeFunction, create_X, MSE, R2, scale
...
MSE_ = np.zeros((2, len(poly_degrees)))
...
for idx, degree in enumerate(poly_degrees):
    ...
    MSE_[0, idx] = MSE(z_train, z_predict_train)
    MSE_[1, idx] = MSE(z_test, z_predict_test)
    r2[0, idx] = R2(z_train, z_predict_train)
    r2[1, idx] = R2(z_test, z_predict_test)


fig, axes = plt.subplots(1, 2)
ax = axes[0]
ax.set_title('Mean Square Error')
ax.plot(poly_degrees, MSE_[0], label="Training data")
ax.plot(poly_degrees, MSE_[1], label="Test data")
