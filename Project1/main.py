...
error_of_lambda, variance_of_lambda, bias_variance_of_lambda, bias_variance, model_terrain, confidence_intervall, bias_variance_comp, error_of_polydeg_comp)
...
def produce_terrain():
    ...
    *_, X_array, beta_array = model.simple_regression(x, y, z, 4.28e-2, False)
    ...
    beta = beta_array[deg_idx]
    model_terrain(X, x, y, beta, N,
    title = f'Ridge regression with p={deg_idx + 1}',
    fname = f'plots/Ridge/Ridge_surf_p{deg_idx + 1}_N{N}.pdf'
    )
...
def produce_results_OLS():
    ...
    deg_idx = 3
    confidence_intervall(X_array[deg_idx], beta_array[deg_idx], N,
    title=r'$\beta$ values with corresponding confidence interval of 95$\%$,' + f'p={deg_idx+1}, n={N}',
    fname=f'conf_plot_p{deg_idx+1}_N{N}.png'
    )
    MSE_R2(
