...
def produce_results_OLS():
    ...
    MSE_R2(
        ...
        title='OLS regression without resampling',
...
def produce_results_ridge():
    ...
    bias_variance(
        poly_degrees, error, bias, variance,
        title=r'Ridge regression without resampling for $\lambda = $' + f'{lmb}',
    ...
    error_of_polydeg(
        ...
        fname=f'plots/exercise4_k-fold_{k}_lmb_{lambdas[lmb_idx]:.2e}_{N}.pdf'
    )
    owncode_vs_sklearn(poly_degrees, error[:, :, lmb_idx], error_sklearn[:, lmb_idx],
        ...
        fname=f'plots/exercise4_k-fold_sklearn_{k}_lmb_{lambdas[lmb_idx]:.2e}_{N}.pdf')

    error_of_lambda(
        lambdas, error[:, deg_idx],
        title=f'Ridge regression with k-foldning for polynomial degree {poly_degrees[deg_idx]}',
        fname=f'plots/exercise4_k-fold_{k}_lambda_deg_{poly_degrees[deg_idx]:.2e}_{N}.pdf'
    )
...
def produce_results_lasso():
    ...
    bias_variance(
        poly_degrees, error, bias, variance,
        title=r'Lasso regression without resampling for $\lambda = $' + f'{lmb}',
    ...
    error_of_polydeg(
        ...
        fname=f'plots/exercise5_k-fold_{k}_lmb_{lambdas[lmb_idx]:.2e}_{N}.pdf'
    )

    owncode_vs_sklearn(poly_degrees, error[:, :, lmb_idx], error_sklearn[:, lmb_idx],
        title='Lasso regression with k-foldning for $\lambda = $' + f'{lambdas[lmb_idx]:.2e}',
        fname=f'plots/exercise5_k-fold_sklearn_{k}_lmb_{lambdas[lmb_idx]:.2e}_{N}.pdf'
    )

    error_of_lambda(
        lambdas, error[:, deg_idx],
        title=f'Lasso regression with k-foldning for polynomial degree {poly_degrees[deg_idx]}',
        fname=f'plots/exercise5_k-fold_{k}_lambda_deg_{poly_degrees[deg_idx]:.2e}_{N}.pdf'
    )
