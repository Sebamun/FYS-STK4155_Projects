    ...
    for n_layers in n_layers_list:
        for n_hidden_neurons in n_hidden_neurons_list:
            for lmbd in lmbd_list:
                ...
                sigmoid_model.train(X_train, z_train, epochs, batch_size)
                time_sigmoid = time.time() #- time_sigmoid
                z_h, a_h, z_o_sigmoid, a_L = sigmoid_model.feed_forward(X_test)
                MSE_sigmoid = np.mean((z_test - z_o_sigmoid)**2)
                f.write(f'Sigmoid   |  {(time_sigmoid-start):.1f} | {MSE_sigmoid:.3f}|  {lmbd}  |     {eta}     |   {n_layers}    |        {n_hidden_neurons}         |   {batch_size}   |      |   {epochs}   | \n')
# for n_layers in n_layers_list:
#     sigmoid_model = Sigmoid(eta, t1, lmbd, gamma, n_layers, n_hidden_neurons, X_train, 'regression')
#     sigmoid_model.train(X_train, z_train, epochs, batch_size)
#     time_sigmoid = time.time() - time_sigmoid
#     z_h, a_h, z_o_sigmoid, a_L = sigmoid_model.feed_forward(X_test)
#     MSE_sigmoid = np.mean((z_test - z_o_sigmoid)**2)
...
TANH_model.train(X_train, z_train, epochs, batch_size)
...
RELU_model.train(X_train, z_train, epochs, batch_size)
time_relu = time.time()
...
ELU_model.train(X_train, z_train, epochs, batch_size)
...
HS_model.train(X_train, z_train, epochs, batch_size)
...
Leaky_model.train(X_train, z_train, epochs, batch_size)
