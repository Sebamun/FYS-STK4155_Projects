...
# lmbd_list = [5e-4]
# n_layers_list = [6,7,8,9,10] #,2,3,4]#,5,6]
# n_hidden_neurons_list = [15] #, 10, 20]
# for lmbd in lmbd_list:
#     for batch_size in batch_size_list:
#         for n_layers in n_layers_list:
#             for n_hidden_neurons in n_hidden_neurons_list:
...
# quit()
# t0, t1 = [1e-5], 100 # Paramters used in learning rate.
# lmbd_list = [5e-4]
# for lmbd in lmbd_list:
#     for batch_size in batch_size_list:
#         for n_layers in n_layers_list:
...
#                 for eta in t0:
#                     start = time.time()
#                     Leaky_model = Leaky(t0, t1, lmbd, gamma, tol, n_layers, n_hidden_neurons, X_train, 'regression')
...
#                     f.write(f'Leaky     | {(time_leaky-start):.1f} | {MSE_leaky:.3f}|  {lmbd}  |     {eta}     |   {n_layers}    |        {n_hidden_neurons}         |   {epochs}   | \n')
...
#                 for eta in t0:
#                     start = time.time()
#                     HS_model = Heaviside(t0, t1, lmbd, gamma, tol, n_layers, n_hidden_neurons, X_train, 'regression')
...
#                     f.write(f'Heaviside | {(time_HS-start):.1f} | {MSE_HS:.3f}|  {lmbd}  |     {eta}     |   {n_layers}    |        {n_hidden_neurons}         |   {epochs}   | \n')
...
#                 for eta in t0:
#                     start = time.time()
#                     ELU_model = ELU(t0, t1, lmbd, gamma, tol, n_layers, n_hidden_neurons, X_train, 'regression')
...
#                     f.write(f'Elu       | {(time_elu-start):.1f} | {MSE_elu:.3f}|  {lmbd}  |     {eta}     |   {n_layers}    |        {n_hidden_neurons}         |   {epochs}   | \n')
